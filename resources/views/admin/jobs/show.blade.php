                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $jobs->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $jobs->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $jobs->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($jobs->status==0)?'InActive':(($jobs->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$jobs->role_title}}</label>
                            </div>
                            @if($jobs->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$jobs->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('JobsController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($jobs->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($jobs->profile_image != ''){{ asset($jobs->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
