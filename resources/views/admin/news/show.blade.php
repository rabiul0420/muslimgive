                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $news->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $news->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $news->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($news->status==0)?'InActive':(($news->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$news->role_title}}</label>
                            </div>
                            @if($news->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$news->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('NewsController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($news->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($news->profile_image != ''){{ asset($news->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
