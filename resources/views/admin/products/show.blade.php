                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $products->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $products->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $products->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($products->status==0)?'InActive':(($products->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$products->role_title}}</label>
                            </div>
                            @if($products->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$products->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('ProductsController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($products->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($products->profile_image != ''){{ asset($products->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
