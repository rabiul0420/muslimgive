                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $products_type->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $products_type->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $products_type->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($products_type->status==0)?'InActive':(($products_type->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$products_type->role_title}}</label>
                            </div>
                            @if($products_type->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$products_type->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('ProductsTypeController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($products_type->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($products_type->profile_image != ''){{ asset($products_type->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
