                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $payments->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $payments->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $payments->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($payments->status==0)?'InActive':(($payments->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$payments->role_title}}</label>
                            </div>
                            @if($payments->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$payments->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('PaymentsController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($payments->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($payments->profile_image != ''){{ asset($payments->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
