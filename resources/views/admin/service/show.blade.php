                ...
                <div class="col-xs-9">
                    <div class="box">
                        ...
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                <label for="email">Email: {{ $service->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $service->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($service->status==0)?'InActive':(($service->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$service->role_title}}</label>
                            </div>
                            @if($service->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$service->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('ServiceController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($service->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($service->profile_image != ''){{ asset($service->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
