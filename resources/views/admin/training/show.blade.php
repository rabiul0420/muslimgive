                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $training->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $training->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $training->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($training->status==0)?'InActive':(($training->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$training->role_title}}</label>
                            </div>
                            @if($training->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$training->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('TrainingController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($training->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($training->profile_image != ''){{ asset($training->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
