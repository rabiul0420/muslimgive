                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $menu->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $menu->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $menu->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($menu->status==0)?'InActive':(($menu->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$menu->role_title}}</label>
                            </div>
                            @if($menu->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$menu->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('MenuController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($menu->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($menu->profile_image != ''){{ asset($menu->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
