                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $pages->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $pages->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $pages->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($pages->status==0)?'InActive':(($pages->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$pages->role_title}}</label>
                            </div>
                            @if($pages->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$pages->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('PagesController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($pages->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($pages->profile_image != ''){{ asset($pages->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
