                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $video->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $video->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $video->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($video->status==0)?'InActive':(($video->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$video->role_title}}</label>
                            </div>
                            @if($video->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$video->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('VideoController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($video->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($video->profile_image != ''){{ asset($video->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
