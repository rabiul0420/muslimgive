                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $charity_profile->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $charity_profile->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $charity_profile->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($charity_profile->status==0)?'InActive':(($charity_profile->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$charity_profile->role_title}}</label>
                            </div>
                            @if($charity_profile->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$charity_profile->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('CharityProfileController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($charity_profile->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($charity_profile->profile_image != ''){{ asset($charity_profile->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
