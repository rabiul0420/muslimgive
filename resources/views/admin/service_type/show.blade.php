                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $service_type->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $service_type->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $service_type->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($service_type->status==0)?'InActive':(($service_type->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$service_type->role_title}}</label>
                            </div>
                            @if($service_type->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$service_type->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('ServiceTypeController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($service_type->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($service_type->profile_image != ''){{ asset($service_type->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
